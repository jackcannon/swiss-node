...
var setOutputModeFn = (mode) => {
  if (mode === void 0)
    mode = outputMode;
  const args = {
    mode: import_swiss_ak2.safe.str(mode, false, "ANSI")
  };
  if (args.mode === "AUTO") {
    ...
    ...Object.fromEntries(
      Object.entries(colourStyleConfigs).flatMap(([key, value]) => {
        ...
        return [
          ...
          ["dark" + import_swiss_ak2.StringTools.capitalise(colourName, false) + bgSuffix, [...value.dark]],
          ["light" + import_swiss_ak2.StringTools.capitalise(colourName, false) + bgSuffix, [...value.light]]
        ];
      })
    ),
    ...blackStyleConfigs,
...
var debugFn = (text2) => {
  ...
  return Object.entries(debugReplacements).reduce((txt, [search, replace]) => import_swiss_ak2.StringTools.replaceAll(txt, search, replace), args.text);
};
var getColrFn = (name, styles = [], options) => {
  ...
  const result = (...text2) => {
    ...
    const pairs = import_swiss_ak2.ArrayTools.zipMax(flatStarts, flatEnds);
    output = pairs.reduceRight((txt, pair) => {
      let start = getOutputForCodes(pair[0]);
      let end = getOutputForCodes(pair[1]);
      return import_swiss_ak2.StringTools.replaceAll(txt, end, end + start);
    }, output);
    output = output.replace(/\r?\n/g, (match) => `${suffix}${match}${prefix}`);
    output = prefix + output + suffix;
    return output;
  };
  const colrFnCache = import_swiss_ak.cachier.create();
  Object.defineProperties(
    result,
    import_swiss_ak2.ObjectTools.mapValues(optionConfigs, (key, value) => ({
      enumerable: false,
  ...
  Object.defineProperties(
    result,
    import_swiss_ak2.ObjectTools.mapValues(fullStyleConfigs, (key, value) => ({
      enumerable: true,
  ...
  const templateFn = (strings, ...exps) => {
    const args = {
      strings: import_swiss_ak2.safe.arrOf.str([...strings]),
      exps: import_swiss_ak2.safe.arr(exps)
    };
    const styledExps = args.exps.map((v) => result(v));
    const zipped = import_swiss_ak2.ArrayTools.zipMax(args.strings, styledExps);
  ...
  const setsCache = import_swiss_ak.cachier.create();
  Object.defineProperties(result, {
    sets: {
      enumerable: false,
      get: () => setsCache.getOrRun("sets", () => {
        const setCache = import_swiss_ak.cachier.create();
        return Object.defineProperties(
          {},
          import_swiss_ak2.ObjectTools.mapValues(setConfigs, (name2, [textKey, bgKey]) => ({
            enumerable: true,
            get: () => setCache.getOrRun(name2, () => {
              const fnCache = import_swiss_ak.cachier.create();
              return Object.defineProperties({}, {
                text: {
                  enumerable: true,
...
((LogTools2) => {
  ...
  LogTools2.processLogContents = (prefix, wrapper = import_swiss_ak3.fn.noact, ...args) => args.map(LogTools2.getLogStr).join(" ").split("\n").map((line, index) => colr.bold(index ? " ".repeat(prefix.length) : prefix) + " " + wrapper(line)).join("\n");
  LogTools2.getLog = (prefix, wrapper = import_swiss_ak3.fn.noact) => (...args) => {
...
var getScrollbar = (allItems, scrolledItems, theme, height = scrolledItems.items.length, trimmedEndTop = false, trimmedEndBottom = false) => {
  ...
  const scrollbarBar = import_swiss_ak8.ArrayTools.repeat(barHeight, barIcon);
  ...
  const result = [...import_swiss_ak8.ArrayTools.repeat(trackStartHeight, trackIcon), ...scrollbarBar, ...import_swiss_ak8.ArrayTools.repeat(trackEndHeight, trackIcon)];
  if (trimmedEndTop) {
...
var getActionBar = (ids, config, pressedId, disabledIds = []) => {
  const keyList = ids.filter(import_swiss_ak16.fn.isTruthy).filter((key) => config[key]);
...
((PathTools2) => {
  PathTools2.explodePath = (path2) => {
    const args = {
      path: import_swiss_ak22.safe.str(path2)
    };
...
var extraInfo = (0, import_swiss_ak23.onDemand)({
  isMacOS: () => process.platform === "darwin"
});
var caches = (0, import_swiss_ak23.onDemand)({
  isMacOSAlias: () => import_swiss_ak23.cachier.create((0, import_swiss_ak23.minutes)(1)),
  resolveMacOSAlias: () => import_swiss_ak23.cachier.create((0, import_swiss_ak23.minutes)(1)),
  getActualLocationPath: () => import_swiss_ak23.cachier.create((0, import_swiss_ak23.minutes)(1)),
  clear: () => () => {
...
var isMacOSAlias = (path2) => {
  ...
  return caches.isMacOSAlias.getOrRun(path2, () => {
    ...
    try {
      const fileDescriptor = import_fs.default.openSync(path2, "r");
      const headerBuffer = new Uint8Array(16);
      try {
        import_fs.default.readSync(fileDescriptor, headerBuffer, 0, 16, 0);
        const fileHeader = Buffer.from(headerBuffer).toString("hex");
        const isAlias = fileHeader === ALIAS_HEADER;
        return isAlias;
      } finally {
        import_fs.default.closeSync(fileDescriptor);
      }
    } catch (error) {
      return false;
    }
...
var resolveMacOSAliasFallback = async (actualPath) => {
  ...
  `;
      ...
      try {
        ...
        const destination = PathTools.removeTrailSlash(stdout.trim());
        if (destination && destination !== absolutePath && import_fs.default.existsSync(destination)) {
          return destination;
        }
      } catch (fallbackError) {
      }
...
var humanFileSize = (size) => {
  ...
  return import_swiss_ak24.MathsTools.roundTo(0.01, size / Math.pow(1024, i)) * 1 + " " + ["B", "kB", "MB", "GB", "TB"][i];
};
var getFilePanel = (path2, panelWidth, maxLines) => {
  ...
  result.push(out.center(col.specialFaded(`${ext.toUpperCase()} ${category ? `${import_swiss_ak24.StringTools.capitalise(category)} ` : ""}File`), panelWidth));
  ...
  const addTimeItem = (title, time2, append) => {
    addItem(title, `${import_swiss_ak24.TimeTools.toReadableDuration(now - time2, false, 2)}${append || ""}`);
  };
  if (stat) {
  ...
  if (info) {
    ...
    if (["video", "audio"].includes(category) && info.duration)
      addItem(`Duration`, import_swiss_ak24.TimeTools.toReadableDuration((0, import_swiss_ak24.seconds)(info.duration), false, 2));
